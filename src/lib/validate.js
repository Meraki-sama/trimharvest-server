// ============================================================================
// lib/validate.js — Input validation terpusat pakai Joi.
// Standar industri: JANGAN percaya req.body mentah. Tiap endpoint punya
// schema; kalau gagal validasi, langsung 400 dengan pesan jelas (bukan
// sampai crash di logika bisnis / bocor stack trace). Menghindari bug
// anomali akibat payload tidak terduga dari client/gateway.
// ============================================================================
const Joi = require('joi');

function validate(schema) {
  // Kembalikan Express middleware yang memvalidasi req.body terhadap schema.
  // schema bisa Joi object schema biasa.
  return (req, res, next) => {
    const { error, value } = schema.validate(req.body, {
      stripUnknown: true, // buang field yang tidak dikenal (defense tambahan)
      abortEarly: false,  // kumpulkan semua error, bukan cuma yang pertama
...
const schemas = {
  ...
  changePassword: Joi.object({
    ...
    // ^ Minimal 8 karakter -- standar industri dasar agar password tidak
    //   terlalu lemah (menghindari kasus default 12345678 kebobolan).
  }),
  ...
  command: Joi.object({
    ...
    // field bebas di bawah ini ikut apa adanya (plug & play protocol),
    // tapi kita batasi tipe & panjang agar tidak ada injection aneh.
  ...
  // ^ unknown(false): TOLAK field di luar daftar di atas untuk command
  //   (lebih ketat dari sebelumnya yang terima apa saja). Kalau suatu saat
  //   protocol tambah field, cukup tambahkan di sini.
};
