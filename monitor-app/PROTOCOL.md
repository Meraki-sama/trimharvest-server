...
apapun.** Satu-satunya pihak yang bicara ke PostgreSQL adalah server Node.js,
lewat driver `pg` (connection string `DATABASE_URL` yang hanya ada di server).
Tidak ada client/firmware yang memegang kredensial database — semua akses
data harus lewat API Gateway ini. Lapisan pertahanan kedua ada di sisi
database itu sendiri (user DB dengan hak terbatas, firewall yang membatasi
port hanya dari host server), sehingga kalau suatu saat ada yang salah
menaruh config di tempat yang salah, data tidak langsung terbuka ke publik.
...
gateway & di PostgreSQL server — TIDAK PERNAH dikirim ulang lewat jaringan
...
- `seq` di dalam body lebih kecil atau sama dengan `lastSeq` tersimpan di
 PostgreSQL untuk device itu (anti-replay lapis kedua, di atas
...
```json
{
  ...
  "commands": [
    ...
    { "dest": "gateway", "cmd": "wifi_update", "ssid": "...", "password": "..." }
  ]
}
```

Command `wifi_update` (`dest: "gateway"`) mengganti jaringan WiFi yang dipakai
gateway **dari jarak jauh lewat app**, TANPA perlu HP terhubung fisik ke
Access Point setup gateway (beda dari alur `/configure` di bagian "Mode
Setup" firmware, yang butuh kedekatan fisik). Gateway menyimpan
`ssid`/`password` baru ke NVS lalu restart; kalau kredensial baru gagal
konek, gateway otomatis jatuh ke mode setup (AP) seperti biasa -- jadi
tidak pernah terkunci total hanya karena salah ketik SSID/password.
`device_id`/`device_secret` TIDAK ikut berubah lewat command ini.
...
- `POST /api/auth/login` — `{ username, password }` → `{ accessToken,
  ...
  PostgreSQL — server ini satu-satunya penjaga gerbang, sesuai
  arsitektur di atas).
...
- `POST /api/devices/:id/rekey` — ganti `device_secret` (device_id TETAP
  ...
  PostgreSQL) — server generate secret baru, antre command `rekey` untuk
