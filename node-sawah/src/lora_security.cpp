...
bool loraSecureUnwrap(const uint8_t *frame, size_t frameLen,
    ...
    // Perbandingan MAC konstan-waktu (constant-time) -- ganti memcmp()
    // biasa (#8) supaya tidak bocor informasi lewat perbedaan waktu
    // eksekusi antar byte. XOR seluruh 8 byte lalu OR-kan hasilnya; hasil
    // 0x00 berarti semua byte cocok. Tidak ada short-circuit.
    uint8_t diff = 0;
    for (uint8_t i = 0; i < 8; i++) {
      diff |= expectedMac[i] ^ frame[5 + bodyLen + i];
    }
    if (diff != 0) {
